...
let initTemplateAndGoogleMaps = function (tpl) {
    ...
    if (tpl.data.atts.searchBox) {
        ...
        google.maps.event.addListener(searchBox, 'places_changed', function () {
            ...
            tpl.setMarker(tpl.map, latLng);
            tpl.map.setCenter(latLng);

            if (typeof tpl.data.atts.onPlaceChanged === 'function') {
                tpl.data.atts.onPlaceChanged({
                    address: address,
                    coordinates: latLng
                });
            }
    ...
    if (typeof tpl.data.atts.onRendered === 'function') {
        tpl.data.atts.onRendered(tpl.map);
    }
    ...
    google.maps.event.addListener(tpl.map, 'click', function (e) {
        ...
        geoCoder.geocode({ location: e.latLng }, function (res, status) {
            if (status == "OK" && res[0]) {
                if (input) {
                    $(input).val(res[0].formatted_address);
                }

                if (typeof tpl.data.atts.onPlaceChanged === 'function') {
                    tpl.data.atts.onPlaceChanged({
                        address: res[0].formatted_address,
                        coordinates: res[0].geometry.location
                    });
                }
            }
        });
